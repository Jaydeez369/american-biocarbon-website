/* ===== VEJ Sales OS - INSTANTLY SNAPSHOT (GENERATED, DO NOT EDIT) =====
   Written by sales-department/instantly-analytics/build-instantly-snapshot.mjs from the live
   Instantly API. Sales OS is static and cannot call Instantly at render time, so every
   number here is a dated read and the screen renders the date beside it.

   Re-stamp after any campaign, lead or mailbox change:
     node sales-department/instantly-analytics/build-instantly-snapshot.mjs

   Or refresh every snapshot at once:
     sales-department/refresh-snapshots.sh

   All reads. Nothing here writes to Instantly.
   ===================================================================== */
window.INSTANTLY_LIVE = {
  "read": "2026-09-02 05:46 UTC",
  "readDate": "2026-09-02",
  "source": "snapshot",
  "inWorkspace": 15,
  "launched": 4,
  "launchedNames": [
    "AB.CIVIL",
    "AB.DIST",
    "AB.ENV",
    "AB.OG"
  ],
  "paused": 2,
  "pausedNames": [
    "AB.HDD.SUP",
    "BC-NURS"
  ],
  "drafts": 5,
  "unmanageable": 0,
  "ready": 0,
  "readyLeads": 0,
  "totals": {
    "campaigns": 15,
    "leads": 595,
    "contacted": 1000,
    "sent": 1007,
    "replies": 10,
    "bounced": 5,
    "unsubscribed": 0
  },
  "byLine": {
    "absorbent": {
      "campaigns": 9,
      "leads": 367,
      "contacted": 582,
      "sent": 583,
      "replies": 3,
      "bounced": 1,
      "unsubscribed": 0
    },
    "biochar": {
      "campaigns": 6,
      "leads": 228,
      "contacted": 418,
      "sent": 424,
      "replies": 7,
      "bounced": 4,
      "unsubscribed": 0
    }
  },
  "mailboxes": 4,
  "mailboxDomains": {
    "pureamericanbiocarbon.com": 2,
    "getamericanbiocarbon.com": 2
  },
  "dailyCeiling": 120,
  "requestedDaily": 80,
  "campaigns": [
    {
      "name": "AB.BED.SUP · Bedding Channel · R1",
      "icp": "AB.BED.SUP",
      "line": "absorbent",
      "status": "draft",
      "parked": true,
      "leads": 0,
      "sent": 0,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.CIVIL · Heavy Civil, Dredging and Slurry · R1",
      "icp": "AB.CIVIL",
      "line": "absorbent",
      "status": "live",
      "parked": false,
      "leads": 48,
      "sent": 119,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.DIST · Absorbent Distributors and Safety Supply · R1",
      "icp": "AB.DIST",
      "line": "absorbent",
      "status": "live",
      "parked": false,
      "leads": 52,
      "sent": 127,
      "replies": 1,
      "bounced": 0
    },
    {
      "name": "AB.ENV · Spill Response and Remediation · R1",
      "icp": "AB.ENV",
      "line": "absorbent",
      "status": "live",
      "parked": false,
      "leads": 26,
      "sent": 76,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.HDD.CON · Directional Drilling and Boring Contractors · R1",
      "icp": "AB.HDD.CON",
      "line": "absorbent",
      "status": "completed",
      "parked": false,
      "leads": 11,
      "sent": 33,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.HDD.SUP · Drilling Fluid, Tooling and Boring Supply · R1",
      "icp": "AB.HDD.SUP",
      "line": "absorbent",
      "status": "paused",
      "parked": false,
      "leads": 25,
      "sent": 25,
      "replies": 1,
      "bounced": 0
    },
    {
      "name": "AB.LF · Landfill and Leachate Operations · R1",
      "icp": "AB.LF",
      "line": "absorbent",
      "status": "completed",
      "parked": false,
      "leads": 14,
      "sent": 42,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.MUNI · Municipal Public Works · R1",
      "icp": "AB.MUNI",
      "line": "absorbent",
      "status": "draft",
      "parked": true,
      "leads": 0,
      "sent": 0,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "AB.OG · Oil and Gas Field Services · R1",
      "icp": "AB.OG",
      "line": "absorbent",
      "status": "live",
      "parked": false,
      "leads": 191,
      "sent": 161,
      "replies": 1,
      "bounced": 1
    },
    {
      "name": "BC-BLEND · Soil & Media Blenders · R1",
      "icp": "BC-BLEND",
      "line": "biochar",
      "status": "draft",
      "parked": true,
      "leads": 0,
      "sent": 0,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "BC-COMP · Commercial Composters · R1",
      "icp": "BC-COMP",
      "line": "biochar",
      "status": "draft",
      "parked": true,
      "leads": 0,
      "sent": 0,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "BC-DIST · Ag Distributors & Co-ops · R1",
      "icp": "BC-DIST",
      "line": "biochar",
      "status": "draft",
      "parked": true,
      "leads": 0,
      "sent": 0,
      "replies": 0,
      "bounced": 0
    },
    {
      "name": "BC-NURS · Landscape & Nursery · R1",
      "icp": "BC-NURS",
      "line": "biochar",
      "status": "paused",
      "parked": false,
      "leads": 180,
      "sent": 284,
      "replies": 5,
      "bounced": 2
    },
    {
      "name": "BC.FARM · Row Crop and Specialty Farms · R1",
      "icp": "BC.FARM",
      "line": "biochar",
      "status": "completed",
      "parked": false,
      "leads": 47,
      "sent": 137,
      "replies": 2,
      "bounced": 2
    },
    {
      "name": "BC.FARM.ROLE · Farm Published Inboxes · R1",
      "icp": "BC.FARM.ROLE",
      "line": "biochar",
      "status": "completed",
      "parked": false,
      "leads": 1,
      "sent": 3,
      "replies": 0,
      "bounced": 0
    }
  ],
  "oversubscribed": false,
  "generator": "sales-department/instantly-analytics/build-instantly-snapshot.mjs"
};
