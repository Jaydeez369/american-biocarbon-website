/* GENERATED by scripts/build-nurture.mjs from functions/api/_email.js on 2026-09-16. Edit the source. */
(function(){
/**
...
/* `note` is the one line under the address that says why they are getting this. The default
   is the website form reason. Nurture sequences pass their own, plus an unsubscribe link:
   Gmail and Yahoo require a visible one on anything that is not transactional. */
function footer(env, note, unsubscribeHref) {
  const o = origin(env);
  return (
    ...
    esc(note || "You are receiving this because you submitted a request on our website.") +
    (unsubscribeHref
      ? ` <a href="${esc(unsubscribeHref)}" style="color:#8b9099;text-decoration:underline">Unsubscribe</a>`
      : "") +
    `</p>` +
    `</td></tr>`
  );
 ...
 */
function renderEmail({ style = "classic", preheader = "", heading = "", body = "", env = {}, footerNote = "", unsubscribeHref = "" }) {
  ...
  return (
    ...
    footer(env, footerNote, unsubscribeHref) +
 ...
 */
function buildAutoreply(formKey, fields = {}, env = {}, styleOverride) {
 ...
 */
function buildInternalLead(formKey, fields = {}, page = "", env = {}) {
  const seq = SEQUENCES[formKey];
 ...
 * Nurture sequences.
 *
 * The copy for these is NOT in this file. It is written by Victor and Daniel in the Sales OS
 * (Execute > Nurture Plan), saved to D1 under kind "nurture", and rendered through this one
 * function, both for the preview they approve and for the send. Same rule as the auto reply:
 * one renderer, so the preview is evidence.
 *
 * `copy` is the record the editor saves:
 *   { subject, preheader, heading, greeting, paras:[...], ctaLabel, ctaHref, imageUrl,
 *     imageAlt, signoff, style, footerNote }
 * `to` is who it is going to: { firstName, unsubscribeHref }. Tokens: {{firstName}} in any
 * string is replaced, and a greeting with no first name to put in it is dropped rather than
 * shipped as "Hi ,", which is the bug that made the Instantly campaigns look broken.
 * ------------------------------------------------------------------ */
const nurtureFooterDefault =
  "You are receiving this because you have been in touch with American BioCarbon.";

function buildNurtureEmail(copy = {}, to = {}, env = {}) {
  const first = String(to.firstName || "").trim();
  const fill = (s) => String(s == null ? "" : s).replace(/\{\{\s*firstName\s*\}\}/g, first);
  const hasToken = (s) => /\{\{\s*firstName\s*\}\}/.test(String(s || ""));

  const greetingRaw = String(copy.greeting || "").trim();
  const greeting = greetingRaw && !(hasToken(greetingRaw) && !first) ? fill(greetingRaw) : "";
  const parasList = (copy.paras || []).map((p) => fill(p).trim()).filter(Boolean);
  const ctaLabel = fill(copy.ctaLabel || "").trim();
  const ctaHref = String(copy.ctaHref || "").trim();
  const signoff = fill(copy.signoff || "").trim();
  const imageUrl = String(copy.imageUrl || "").trim();

  const P = (t) => `<p style="margin:0 0 16px;font:400 16px/1.6 ${FONT};color:${C.slate}">${esc(t)}</p>`;
  const image = imageUrl
    ? `<img src="${esc(imageUrl)}" alt="${esc(copy.imageAlt || "")}" width="536" ` +
      `style="display:block;border:0;width:100%;max-width:536px;height:auto;border-radius:6px;margin:0 0 18px">`
    : "";
  const body =
    (greeting ? P(greeting) : "") +
    image +
    parasList.map(P).join("") +
    (ctaLabel && ctaHref ? button(ctaLabel, ctaHref) : "") +
    (signoff
      ? `<p style="margin:22px 0 0;font:400 16px/1.6 ${FONT};color:${C.slate}">${esc(signoff).replace(/\n/g, "<br>")}</p>`
      : "") +
    `<div style="height:6px"></div>`;
  ...
  const text = [
    ...
    BRAND.address,
    BRAND.phone,
    to.unsubscribeHref ? `Unsubscribe: ${to.unsubscribeHref}` : "",
  ]
    .filter((l, i, a) => !(l === "" && a[i - 1] === ""))
    .join("\n")
    .trim();
  ...
  return {
    ...
    html: renderEmail({
      style: copy.style || "classic",
      preheader: fill(copy.preheader || ""),
      heading: fill(copy.heading || ""),
      body,
      env,
      footerNote: copy.footerNote || nurtureFooterDefault,
      unsubscribeHref: to.unsubscribeHref || "",
    }),
    text,
  };
}

const _internals = { C, BRAND, FONT, paras, linkList, button, submissionTable };

window.EMAIL_TPL = { esc, origin, SPEC, STYLES, renderEmail, SEQUENCES, buildAutoreply, buildInternalLead, buildNurtureEmail, _internals };
})();
